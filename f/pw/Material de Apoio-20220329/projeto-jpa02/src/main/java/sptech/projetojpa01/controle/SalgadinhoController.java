...
//    return ResponseEntity.status(404).build();
  ...
  @PatchMa  pping("/{codigo}/preco/{novoPreco}")
  public ResponseEntity patchSalgadinho(@PathVariable Integer codigo, @PathVariable Double novoPreco) {
    if(repository.existsById(codigo)) {
      repository.atualizaPreco(codigo, novoPreco);
      return ResponseEntity.status(200).build();
    }
  ...
  @GetMapping("/{codigo}/preco-apimentado")
  public ResponseEntity patchSalgadinhoMelhorado(
          @PathVariable Integer codigo,
